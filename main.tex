...
\begin{figure}[h]
\centering
\includegraphics[width=1\linewidth]{figure/LLM-system_model_version_v7.drawio.png}
...
\begin{itemize}
  \item 1. Analyze Input files - The AI coding assistant first analyzes the workable configuration content of the input and determines the parameters that can be modified and their types.
  \item 2. Generate CMS - The AI coding assistant generates a config mutation set (CMS) based on the understanding in step one. The key structure is shown in Table \ref{table:Definition_of_Config_Mutation_Set} , and the format is shown in output schema in the appendix \ref{app:gen_prompt}.
  \item 3. Generate modified configuration - Based on the modified\_key and error\_value in the CMS generated in Generate CMS, modify the workable configuration to generate modified configuration.
\end{itemize}
}
...
\begin{enumerate}
  ...
  \item Log Filtering and Keeping The management platform receives logs from the RAN, performs filtering, and only keeps error logs.
\end{enumerate}
}
