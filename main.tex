...
\begin{itemize} 
    ...
    \item \textbf{Modified Configurations ($C_{\text{mod}}$):} A set of configuration files generated by the $AI_{\text{code}}$ based on Workable Configurations
...
\color{blue}{
The purpose of the Synthetic Data Generation component is to construct training datasets for SFT. We adopt a three-stage process as illustrated in Fig. \ref{LLM_reasoning_framework} under Synthetic Data Generation:
}
...
\begin{itemize}
  ...
  \item Generate CMS - $AI_{\text{code}}$ generates a config mutation set (CMS) based on the understanding in Analyze Input Files step. The CMS structure is shown in Table \ref{table:Definition_of_Config_Mutation_Set} , and the format is shown in output schema in the appendix \ref{app:gen_prompt}.
  \item Filter Duplicate Cases - Compare cases in the CMS during generation with historical records to filter out duplicate cases.
  \item Generate $C_{\text{mod}}$ - Based on the \texttt{modified\_key} and \texttt{error\_value} in the CMS generated in Generate CMS step, modify the $C_{\text{work}}$ to generate $C_{\text{mod}}$.
\end{itemize}

To ensure that the model understands the task objectives and response logic precisely, prior work \cite{network_configuration_cn} provides a structure that structure prompt messages into four core elements: role, task description, background context, and expected behavior. Table \ref{tab:prompt_configuration_mutation} presents the $P_{\text{mut}}$ architecture we using in this study in Reasoning Trace.}
...
\textcolor{blue}{This stage is responsible for converting the $C_\text{mod}$ and $L_\text{error}$ generated into Reasoning Traces throughput the $LLM_{Reason}$ which is the training data used for SFT in this paper. Table \ref{tab:prompt_reasoning_trace} presents the prompt structure adopted in this study in Reasoning Trace.}
...
In this paper, we use Cursor \cite{cursor2025} and GitHub Copilot \cite{copilot2025} to build the pipeline and create error logs and reasoning traces. We adopt OAI \cite{oai} as the RAN testbed (component definitions in the RAN environment are shown in Table \ref{table:oai_testing_environment}), with the O-RAN Software Community (OSC)'s SMO \cite{smo} serving as the management platform (component definitions in SMO are shown in \ref{table:smo}). The experimental architecture as shown in the Fig. \ref{Experimental_Architecture}.

\begin{figure*}
    \centering
    \includegraphics[width=1\linewidth]{figure/Experimental Architecture.png}
...
\begin{table}[H]
    \centering
    \caption{Definition of SMO Component}
    \label{table:oai_testing_environment}
    \begin{tabular}{|m{1.5cm}|m{6.5cm}|}
        \hline
        \textbf{Component} & \textbf{Definition and Purpose} \\
