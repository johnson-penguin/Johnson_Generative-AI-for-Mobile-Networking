...
\begin{itemize} 
    ...
    \item Configuration Mutation Prompt $\ref{app:gen_prompt}$ - Instruct the AI coding assistant on how to create a configuration change set based on available configuration files, including the generated format, tools used, quantity generated, and target components. Users can adjust \texttt{COMPONENT} to select the network element to modify (CU, DU, or UE in this article). Users can also adjust \texttt{N\_VARIATIONS} to control the number of errors generated (this paper recommends 25).
\end{itemize}
}
