...
\begin{itemize} 
    \item \textbf{Workable Configuration ($\text{C}_{\text{work}}$):} A configuration file enabling the gNB to work properly.
    ...
    \item \textbf{AI Coding Assistant ($\text{LLM}_{\text{code}}$):} An agent or AI-assisted tool driven by the $P_{\text{mut}}$, which randomly modifies parameters of a Workable Configuration to generate new configurations.
    ...
    \item \textbf{Reasoning LLM ($\text{LLM}_{\text{reason}}$):} An LLM designed to execute $P_{\text{trace}}$ and generate structured reasoning traces.
        

    \item \textbf{SFT Training Data ($\text{D}_{\text{SFT}}$):} The output from the $LLM_{Reason}$, records the structured reasoning trace for each input $L_\text{error}$ and $\text{C}_{\text{work}}$, and adjustment suggestions.
    
    \item \textbf{Base LLM ($\text{LLM}_{\text{base}}$):}
    \item \textbf{Fine-turned LLM ($\text{LLM}_{\text{sft}}$):}
...
\color{blue}{
This stage is responsible for generating a large number of configuration files with different variations based on input configuration files. Once the $\text{C}_{\text{work}}$ and $P_{\text{mut}}$ are received, the $\text{LLM}_{\text{code}}$ will generate the $C_{\text{mod}}$ according to the following steps:

\begin{itemize}
  \item Analyze Input Files - $\text{LLM}_{\text{code}}$ first analyzes the $\text{C}_{\text{work}}$ and determines the parameters that can be modified and their types.
  \item Generate CMS - $\text{LLM}_{\text{code}}$ generates a config mutation set (CMS) based on the understanding in Analyze Input Files step. The key structure is shown in Table \ref{table:Definition_of_Config_Mutation_Set} , and the format is shown in output schema in the appendix \ref{app:gen_prompt}.
...
\color{blue}{Once the Management Platform receives $C_{\text{mod}}$, it will get Error Logs according to the following steps:
...
\color{blue}{After completing this stage, we will get the $L_\text{error}$ and the corresponding $C_\text{mod}$.}
...
\textcolor{blue}{This stage is responsible for converting the $C_\text{mod}$ and $L_\text{error}$ generated into Reasoning Traces throughput the $LLM_{Reason}$ which is the training data used for SFT in this paper. To ensure that the model understands the task objectives and response logic precisely, prior work \cite{network_configuration_cn} provides an architecture that structure prompt messages into four core elements: role, task description, background context, and expected behavior. Table \ref{tab:prompt_reasoning_trace} presents the prompt architecture adopted in this study in Reasoning Trace.}
...
\begin{table}[H]
    \centering
    \caption{Components of Reasoning Trace Prompting}
