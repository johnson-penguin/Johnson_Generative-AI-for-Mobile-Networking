...
\color{blue}{
This stage is responsible for generating a large number of configuration files with different variations based on input configuration files. The terms used in this stage are defined below:

\begin{itemize} 
    \item Workable Configuration - A configuration file enabling the gNB to work properly.
    \item Configuration Mutation Prompt - A prompt designed to guide the AI coding assistant generates Modified Configurations based on the input Workable Configuration (see Appendix~\ref{app:gen_prompt} for details).
    \item AI Coding Assistant - An agent or AI-assisted tool driven by the Configuration Mutation Prompt, which randomly modifies parameters of a Workable Configuration to generate new configurations.
    \item Modified Configurations – A set of configuration files generated by the AI Coding Assistant based on Workable Configurations

\end{itemize}
}

\color{blue}{
Once the Workable Configuration and Configuration Mutation Prompt are received, the AI Coding Assistant will generate the Modified Configurations according to the following steps:
\begin{itemize}
  ...
  \item Generate Modified Configurations - Based on the modified\_key and error\_value in the CMS generated in Generate CMS, modify the workable configuration to generate modified configuration.
\end{itemize}
}
...
\textcolor{blue}{This stage is responsible for deploying the Modified Configurations generated during the Configuration Generation and Modification stage into the RAN via the management platform interface, executing them, and collecting logs generated by all components within the RAN.}

\color{blue}{Once the Management Platform receives Modified Configurations, it will get Error Logs according to the following steps:

\begin{itemize}
  \item Configuration Deployment - The Operator deploys Modified Configurations to the RAN via the Management Platform.
  ...
  \item Error Logs Selection - The management platform receives logs from the RAN and removes logs that do not have error features. The error feature definitions used in this paper will be detailed in the experimental results section.
\end{itemize}
}
\color{blue}{After completing this stage, we will get the error logs and can clearly determine which configuration setting caused the error based on the configuration file them used.}
...
\textcolor{blue}{This stage is responsible for converting the Modified Configurations and Error Logs generated during the Configuration Generation and Modification stage and the Deployment and Log Processing stage into Reasoning Traces via the Reasoning LLM which is the training data used for SFT in this paper. The terms used in this stage are defined below:}

\begin{itemize} 
    \item Reasoning Trace Prompt - A prompt designed to guide a Reasoning LLM in generating reasoning traces, requiring three input elements: \texttt{logs}, \texttt{network\_config}, and \texttt{misconfigured\_param}.
    \item Reasoning LLM - A model for analyzing 5G network configurations and logs, whose main purpose is to generate Reasoning Traces. It can establish causal relationships between error features in error logs and configuration parameters through logical derivation.
    \item SFT Training Data - 
\end{itemize}
